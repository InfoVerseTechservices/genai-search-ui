...
// import { ArrowUp, ArrowRight, Image as ImageIconLucide, Paperclip, Video as VideoIconLucide, SlidersHorizontal as SlidersHorizontalIcon, Bot as AIChatIcon, Plus } from 'lucide-react';
...
//     const [isAddMoreModalOpen, setIsAddMoreModalOpen] = useState<boolean>(false);
...
//    const modalButtonsStyle = "flex items-center gap-x-2 hover:bg-neutral-100 w-full px-3 py-3 rounded-md text-left";
...
//          {
//           isAddMoreModalOpen && (
//             <AddMoreModal className='absolute bottom-10' isOpen={isAddMoreModalOpen} onClose={() => setIsAddMoreModalOpen(false)}>
// <div>
...
//               <button type="button" onClick={handleImageModeToggle} title="Image Mode" className={modalButtonsStyle} disabled={isAudioModeActive || isVideoModeActive || isAIChatModeActive}>
//                 <ImageIconLucide size={20} />
//                 <span className="text-sm font-medium">Upload Image</span>
//               </button>
//                <button type="button" onClick={handleVideoModeToggle} title="Video Mode" className={modalButtonsStyle} disabled={isImageModeActive || isAudioModeActive || isAIChatModeActive}>
...
//          }
//                      <button onClick={() => { setIsAddMoreModalOpen(true)}}>
             
// <Plus
//     size={28}
//     className={`p-1 rounded-full text-zinc-500 hover:bg-neutral-200 ${
//       isAddMoreModalOpen ? 'bg-neutral-200' : ''
//     }`}
//   />
//             </button>
//           <div className={cn(
...
// --- START: Reusable and Themed UI Components ---

// A consistent, theme-aware Icon Button with a Tooltip
...
// Hook to create the dynamic gradient border style based on the current theme
...
// --- END: Reusable and Themed UI Components ---


const MessageInput = ({
  sendMessage, loading, onImagePromptSubmit, onAudioPromptSubmit, onVideoPromptSubmit, onAIChatSubmit
}: MessageInputProps) => {
  // All state and functionality from your original component is preserved here
  const [message, setMessage] = useState('');
  const [textareaRows, setTextareaRows] = useState(1);
  const [mode, setMode] = useState<'multi' | 'single'>('single');
  const inputRef = useRef<HTMLTextAreaElement | null>(null);
  ...
  // ... and so on for video, chat params ...
  const [aiChatParams, setAIChatParams] = useState<AIChatParams>({ model: "qwen-3", temperature: 0.7, top_p: 1, max_tokens: 1000 });
  ...
  // Make sure you have this at the top-level of your component:
  // const [imageModel, setImageModelState] = useState(process.env.NEXT_PUBLIC_COLOMBO_DEFAULT_MODEL || '');
  return (
    <>
...
<TooltipIconButton onClick={() => setIsAddMoreModalOpen(true)} label="Add" isActive={isAddMoreModalOpen}>
      ...
      {/* All modals for parameter settings remain here, unchanged */}
      {isImageModeActive && (
        <GenericModal isOpen={isImageParamsModalOpen} onClose={() => setIsImageParamsModalOpen(false)} title="Image Generation Settings" size="lg">
          <ImageGenerationPanel
            imageNegativePrompt={imageNegativePrompt} setImageNegativePrompt={setImageNegativePrompt}
